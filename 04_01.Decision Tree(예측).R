...
dtr_recipe <- train_data %>%
  recipe(가격 ~ .) %>%
  step_zv(all_predictors()) %>% 
  # step_normalize(all_numeric()) %>%  # tree형 모형은 표준화 안함
...
# 하이퍼파라미터 그리드 만들기
dtr_grid <- grid_regular(cost_complexity(),
                         min_n(),
                         levels = 5)
dtr_grid
...
# 튜닝 결과 그래프 그리기
autoplot(dtr_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
dtr_best <- dtr_results %>%
  select_best("rsq")

dtr_best

# workflow에 best model 파라미터 세팅
final_workflow <- dtr_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(dtr_best)         # 기본 workflow에 세팅
