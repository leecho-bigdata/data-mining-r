...
# 하이퍼파라미터 그리드 만들기
knnr_grid <- grid_regular(neighbors(),   # 최소값과 최대값 설정
                          dist_power(),
                          weight_func(),
                          levels = 3)
...
# 8.workflow 만들기
knnr_workflow <- workflow() %>% 
  add_model(knnr_model) %>%
  add_recipe(knnr_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(knnr_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
knnr_best <- knnr_results %>%
  select_best("rsq")

knnr_best

# workflow에 best model 파라미터 세팅
final_workflow <- knnr_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(knnr_best)         # 기본 workflow에 세팅
