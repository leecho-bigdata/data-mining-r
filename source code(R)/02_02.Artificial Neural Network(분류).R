...
# 8.workflow 만들기
annc_workflow <- workflow() %>% 
  add_model(annc_model) %>%
  add_recipe(annc_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(annc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
annc_best <- annc_results %>%
  select_best("roc_auc")

annc_best

# workflow에 best model 파라미터 세팅
final_workflow <- annc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(annc_best)         # 기본 workflow에 세팅
