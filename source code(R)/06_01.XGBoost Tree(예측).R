...
# 하이퍼파라미터 그리드 만들기
xgbr_grid <- grid_regular(tree_depth(),
                         min_n(),
                         levels = 5)
xgbr_grid
...
# 8.workflow 만들기
xgbr_workflow <- workflow() %>% 
  add_model(xgbr_model) %>%
  add_recipe(xgbr_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(xgbr_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
xgbr_best <- xgbr_results %>%
  select_best("rsq")

xgbr_best

# workflow에 best model 파라미터 세팅
final_workflow <- xgbr_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(xgbr_best)         # 기본 workflow에 세팅
