# 01_03.회귀분석(예측)_tuning을 이용한 최적모델 구축
...
# 하이퍼파라미터 그리드 만들기
lrr_grid <- tibble(penalty = 10^seq(-4, -1, length.out = 30))
...
# 8.workflow 만들기
lrr_workflow <- workflow() %>% 
  add_model(lrr_model) %>%
  add_recipe(lrr_recipe)   
...
# 기존모델
#lrr_fit <- lrr_workflow %>%    # workflow수정
#  fit(data = train_data)

# 모델 훈련
lrr_results <- lrr_workflow %>% 
...
# 튜닝 결과 그래프 그리기
autoplot(lrr_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
lrr_best <- lrr_results %>%
  select_best("rsq")

lrr_best

# workflow에 best model 파라미터 세팅
final_workflow <- lrr_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(lrr_best)         # 기본 workflow에 세팅
