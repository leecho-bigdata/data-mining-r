...
dtc_recipe <- train_data %>%
  recipe(진단 ~ .) %>%
  step_zv(all_predictors()) %>% 
  #step_normalize(all_numeric()) %>%       # 트리형 모델은 수치형변수 원형 값 사용
...
# 모델 세팅
# cost_complexity : 비용복잡도 (가지치기에 사용)
# tree_depth : 최대 트리 깊이 (성장멈추기에 사용)
# min_n: 한개 노드에 포함된 최소 관측치 수
...
# 하이퍼파라미터 그리드 만들기
dtc_grid <- grid_regular(cost_complexity(),
                         tree_depth(),
                         min_n(),
                         levels = 3)
dtc_grid
...
# 8.workflow 만들기
dtc_workflow <- workflow() %>% 
  add_model(dtc_model) %>%
  add_recipe(dtc_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(dtc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
dtc_best <- dtc_results %>%
  select_best("roc_auc")

dtc_best

# workflow에 best model 파라미터 세팅
final_workflow <- dtc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(dtc_best)         # 기본 workflow에 세팅
...
final_fit %>%                      # tree_fit -> final_fit 
  extract_fit_engine() %>%
  rpart.plot(roundint = FALSE,    # rpart 세팅
             yesno = 2,
             type = 2, 
             extra = 1, 
             split.font = 1, 
...
# 모형 2
rpart_fit <- final_fit %>%         # tree_fit -> final_fit
  extract_fit_parsnip()
