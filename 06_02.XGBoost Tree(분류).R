...
# 하이퍼파라미터 그리드 만들기
xgbc_grid <- grid_regular(tree_depth(),
                         min_n(),
                         levels = 5)
xgbc_grid
...
# 8.workflow 만들기
xgbc_workflow <- workflow() %>% 
  add_model(xgbc_model) %>%
  add_recipe(xgbc_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(xgbc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
xgbc_best <- xgbc_results %>%
  select_best("roc_auc")

xgbc_best

# workflow에 best model 파라미터 세팅
final_workflow <- xgbc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(xgbc_best)         # 기본 workflow에 세팅
