...
# 모델 인자(argument) 확인
# install.packages("kernlab")                 # kernlab 설치
args(svm_rbf) 
...
# 하이퍼파라미터 그리드 만들기
svmc_grid <- grid_regular(cost(c(-1, 1)),
                          rbf_sigma(c(-1, 1)),
                          levels = 5)
...
# 8.workflow 만들기
svmc_workflow <- workflow() %>% 
  add_model(svmc_model) %>%
  add_recipe(svmc_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(svmc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
svmc_best <- svmc_results %>%
  select_best("roc_auc")

svmc_best

# workflow에 best model 파라미터 세팅
final_workflow <- svmc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(svmc_best)         # 기본 workflow에 세팅
