...
# 하이퍼파라미터 그리드 만들기
knnc_grid <- grid_regular(neighbors(),   # 최소값과 최대값 설정
                          dist_power(),
                          weight_func(),
                          levels = 3)
...
# 8.workflow 만들기
knnc_workflow <- workflow() %>% 
  add_model(knnc_model) %>%
  add_recipe(knnc_recipe)   
...
# 튜닝 결과 그래프 그리기
autoplot(knnc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
knnc_best <- knnc_results %>%
  select_best("roc_auc")

knnc_best

# workflow에 best model 파라미터 세팅
final_workflow <- knnc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(knnc_best)         # 기본 workflow에 세팅
