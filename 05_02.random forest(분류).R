...
# 하이퍼파라미터 그리드 만들기
rfc_grid <- grid_regular(mtry(c(1L, 9L)),   # 최소값과 최대값 설정
                         min_n(),
                         levels = 5)
rfc_grid
...
# 튜닝 결과 그래프 그리기
autoplot(rfc_results)                # 튜닝결과 그래프 보기 추가
...
# best model 선택
rfc_best <- rfc_results %>%
  select_best("roc_auc")

rfc_best

# workflow에 best model 파라미터 세팅
final_workflow <- rfc_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(rfc_best)         # 기본 workflow에 세팅
