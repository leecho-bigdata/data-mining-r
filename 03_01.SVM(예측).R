...
# 하이퍼파라미터 그리드 만들기
svmr_grid <- grid_regular(cost(c(-1, 1)),
                          rbf_sigma(c(-1, 1)),
                          levels = 5)
...
# 기존모델
#svmr_fit <- svmr_workflow %>%    # workflow수정
#  fit(data = train_data)

# 모델 훈련
svmr_results <- svmr_workflow %>% 
...
# 튜닝 결과 그래프 그리기
autoplot(svmr_results)                # 튜닝결과 그래프 보기 추가
...
final_workflow <- svmr_workflow %>%    # 기본 workflow에 세팅
  finalize_workflow(svmr_best)         # 기본 workflow에 세팅
